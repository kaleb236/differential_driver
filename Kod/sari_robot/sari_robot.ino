...
void loop() {
  ...
  currentMillis = millis();
        if (currentMillis - previousMillis >= loopTime) {  // run a loop every 10ms          
            ...
            float modifier_lin = 1;        // scaling factor because the wheels are squashy / there is wheel slip etc.
            float modifier_ang = 1;        // scaling factor because the wheels are squashy / there is wheel slip etc.


            forward0 = demandx * (63206 * modifier_lin) ; // convert m/s into counts/s
            forward1 = demandx * (63206 * modifier_lin); // convert m/s into counts/s

            turn0 = demandz * (10977 * modifier_ang);    // convert rads/s into counts/s
            turn1 = demandz * (10977 * modifier_ang);    // convert rads/s into counts/s

            forward0 = forward0*-1;      // one motor and encoder is mounted facing the other way

            odrive1.SetVelocity(0, forward1 + turn0); 
            odrive1.SetVelocity(1, forward0 + turn1);
            ...
            pos0_mm_diff = pos0_diff / 63.20;
            pos1_mm_diff = pos1_diff / 63.20;
            ...
            float phi = ((pos1_mm_diff - pos0_mm_diff) / 350);
            ...
            odom_msg.twist.twist.angular.z = ((pos1_mm_diff - pos0_mm_diff) / 350)*100;      // anglular velocity
